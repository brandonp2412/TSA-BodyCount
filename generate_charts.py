...
OUT = ROOT / "charts"
W, H = 1200, 720
BG = "#07111f"; PANEL = "#0d1b2b"; GRID = "#24364a"; TEXT = "#f8fafc"; MUTED = "#9fb3c8"
BLUE = "#53a7ff"; CYAN = "#2dd4bf"; GOLD = "#f5b942"; RED = "#f05252"; WHITE = "#ffffff"
...
def footer(source):
    return [f'<line x1="60" y1="654" x2="1140" y2="654" stroke="{GRID}"/>', text(60, 684, source, 13, MUTED), '</svg>']


def chart1():
    d = load_inputs(); rows = scenarios(d)
    s = svg_open("TSA LIVES BURNED EACH YEAR", "Each bar uses one federal wait-time measurement · 1 life = 79 years")
    left, top, right, bottom = 95, 165, 1140, 535; max_y = 500
    for tick in range(0, 501, 100):
        y = bottom - tick/max_y*(bottom-top)
        s += [f'<line x1="{left}" y1="{y:.1f}" x2="{right}" y2="{y:.1f}" stroke="{GRID}" stroke-dasharray="5 7"/>', text(left-15, y+5, str(tick), 13, MUTED, "end")]
    colors = [BLUE, BLUE, CYAN, GOLD, RED]; slot=(right-left)/len(rows)
    for i, row in enumerate(rows):
        cx=left+slot*(i+.5); h=row.lifetime_equivalents/max_y*(bottom-top); y=bottom-h
...
def chart4():
    d=load_inputs(); one=lifetime_equivalents(d["tsa_screenings_2024"],1.0,d["life_expectancy_us_2024"])[2]
    s=svg_open("ONE EXTRA TSA MINUTE = 21.8 LIVES A YEAR", "904 million annual screenings turns every added minute into 21.8 full 79-year lives")
    left, top, right, bottom=110,205,1120,535; x_max=25; y_max=550
    for tick in range(0,551,100):
        y=bottom-tick/y_max*(bottom-top)
        s += [f'<line x1="{left}" y1="{y:.1f}" x2="{right}" y2="{y:.1f}" stroke="{GRID}" stroke-dasharray="5 7"/>', text(left-16,y+5,str(tick),13,MUTED,"end")]
    pts=[]
    for minute in range(26):
        equiv=lifetime_equivalents(d["tsa_screenings_2024"],float(minute),d["life_expectancy_us_2024"])[2]
        x=left+minute/x_max*(right-left); y=bottom-equiv/y_max*(bottom-top); pts.append(f"{x:.1f},{y:.1f}")
    s.append(f'<polyline points="{" ".join(pts)}" fill="none" stroke="{GOLD}" stroke-width="7" stroke-linecap="round"/>')
    for minute in [5,10,15,20,25]:
        equiv=one*minute; x=left+minute/x_max*(right-left); y=bottom-equiv/y_max*(bottom-top)
        s.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="8" fill="{RED}" stroke="{WHITE}" stroke-width="2"/>')
        s.append(text(x,y-18,f"{equiv:.0f} lives",16,WHITE,"middle",850))
